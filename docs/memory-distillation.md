...
> 脱敏模块 (`agentmemhub/sanitize.py`) / 两表 DDL / S0 切片器 / S1 段级蒸馏 /
> S2 合并沉淀 / S3+S4 去重投影 / CLI `distill` / 控制台 `[3]` / 面板「蒸馏记忆」/
> 71 题评测（私有评测集，仓库仅带示例）/ 生产库全量整顿 + 增量追加验证（三次运行完全幂等）。
> 面板侧配套（P1–P4）：全局会话 uid 与用户修改保护、记忆报表 API（`/api/memories`）、
> 双标签页前端、表头排序、状态式反馈与 ⭐ 手动加权。
...
### 2.1 分流规则（切分基准 = 固定轮数窗口）

> **为什么不用「累积长度」作切分基准**（曾如此，已改）：统一会话提取是**增量
> 追加**的——按累积长度切会让新对话把后续切点整体推移，历史切片 hash 全变 →
> 大面积无谓重蒸。改为按固定轮数窗口切，切点只依赖窗口内内容，
> **追加新对话时已满窗口的切片逐字不变**（实测 397 轮会话：36 个共同切片中
> 仅含截断点的 1 片变化）。

```
会话正文（user+assistant，is_system=0，排除过滤后）
  ├─ chars ≤ max_chars 且 turns ≤ max_turns → 单片（slice_key='whole'）
  └─ 否则按**固定轮数窗口**切分：
       窗口 k = 轮 [k*max_turns, (k+1)*max_turns)，窗口间互不影响
       窗口内：
       ① 按字符预算在轮边界成片（只在本窗口内累积）
       ② 话题边界细化（topic_boundary=true）：切点 ±boundary_window 轮内，
          按相邻轮的关键词重叠度（3-gram Jaccard，复用 FTS 分词口径）取最低处
          —— **只能在窗口内移动**，不跨窗口追随话题
       ③ 单轮自身超预算（导入型巨会话）：轮内按 max_chars 硬切，对齐行边界
          （不施加 per_message_cap，否则巨轮内容会被截成 2000 字丢掉 99%）

slice_key = `w{窗口号}-{窗口内序号}`（如 `w0-1`）——键本身表达窗口归属，
某窗口内片数变化不会波及其它窗口的键名。
```

**稳定性契约**：已满窗口的切片（键 + content_hash）在后续追加对话时**逐字不变**；
只有包含新轮的尾部未满窗口会重新切分（新内容总要处理，无法避免）。
...
**验收标准（全部满足才算成）**
1. 评测集 71 题（`eval/queries.yaml`，可参照 `queries.example.yaml` 格式）：
   蒸馏后 hit@k / MRR **不低于**基线（理想上升）
